...
class TradeDataPrep:
    ...
    def _set_open_close(self):
        self.hist_df["open_close"] = self.hist_df.apply(lambda x: "Open" if ( x.action in ["BUY_TO_OPEN", "SELL_TO_OPEN"] ) else "Close", axis = 1 )

    def _set_buy_sell(self):
        self.hist_df["buy_sell"] = self.hist_df.apply(lambda x: "Buy" if ( x.action in ["BUY_TO_OPEN", "BUY_TO_CLOSE"] ) else "Sell", axis = 1 )
        self.hist_df.to_csv("buy_sell.csv", index=False)
